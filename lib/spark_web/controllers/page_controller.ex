defmodule SparkWeb.PageController do
  ...
  def lgn(conn, _params) do
  	changeset = Account.change_user(%User{})
    render(conn, "login-nosocial.html", layout: {SparkWeb.LayoutView, "fe.html"}, changeset: changeset)
  end
  ...
  case Account.authenticate_user_front(email, password) do
    ...
    {:error, reason} ->
      ...
      |> render("login-nosocial.html", layout: {SparkWeb.LayoutView, "fe.html"}, changeset: changeset)
    end
  end
